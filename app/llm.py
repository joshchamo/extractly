...
async def get_selectors_from_llm(title: str, accessibility_tree: dict, body_text: str, prompt: str) -> dict:
    ...
    system_prompt = (
        ...
        "- CSS selectors (e.g., '.product-item', 'div.card', 'a.example-tile-div')\n"
        "- Role selectors (e.g., 'role=heading[name=\"My Title\"]', 'role=link')\n"
        "- Text selectors (e.g., 'text=\"My Text\"')\n"
        "- XPath selectors (e.g., '//div[@class=\"card\"]')\n\n"
        ...
        "  ],\n"
        "  \"explanation\": \"Found 24 product listings with name and price fields.\"\n"
        "}"
    )
