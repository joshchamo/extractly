...
async def get_selectors_from_llm(title: str, accessibility_tree: dict, body_text: str, prompt: str) -> dict:
    ...
    system_prompt = (
        ...
        "When extracting text fields (like a name, title, description, price, or text label), you MUST target the element containing the text content. "
        "Always prefer class-based selectors (e.g. '.price_color', '.titleline a', or '.title-text') over generic tag names (e.g. 'strong', 'span', 'p') if the class names are visible in the accessibility tree. "
        "Do NOT guess or assume tags like 'strong' if they are not present in the tree.\n\n"
